...
|---|---|---|
| `@slowcook-ai/cli` | `0.18.0-alpha.7` | entity-first foundation (`init entities` extracts ERD → TS schemas + zod, mock-side re-export shim); brew artifact-prune; recon resolveImport mock-strict; gh-proxy CORS strip |
...
**End-to-end validated (legacy mode):** rewo issue #149 → PR #154 on 2026-05-04 ($1.29, 5 iters, 35/35 green). The plate-mode `@slowcook-port-from` carve-out has been deleted from the roadmap — the entities-hypothesis falsification (see above) showed prop-shape drift can't be retired with structural rails alone. The active architectural cut is **pair-brew** (cli α.8): a navigator agent runs per-iteration alongside the driver, catches design-fidelity / cross-story-risk / reuse concerns the structural test gates miss. Local sim on rewo story-018 validated the loop catches the "garbage that passes tests" failure mode. See task #76 for the ship list.
