...
- **Chef α.10 L2 — finisher mode** (in-repo, post-`0.18.0`): `slowcook chef-drift --pr <number>` checks out the PR's branch, makes its surgical edits there, and pushes back to the PR head — re-running CI without dispatching a fresh brew. New `brew_halt_class` enrichment: cli precomputes `failing_test_files`, `failing_test_contents`, and `source_file_contents` (resolves the failing tests' relative imports + reads them) so chef can write surgical search_replace pairs without read tools. `failing_test_contents` is read-only (tests/ frozen) — if only a test edit can fix the failure, chef returns `pm_question`. Audit comment routes to the PR (not source issue) when in finisher mode. 11 new unit tests on the pure helpers (`parseBrewHaltOutput`, `collectImportedSourceFiles`); end-to-end finisher run on a real halted brew PR remains as the next dogfood gate.
