...
**0.22.0** (2026-06-26) — **whole-app LCR + the EPSS test matrix done right.** `vibe` now builds one navigable **Living Coded Requirement** app (Vite/React over in-browser SQLite) instead of per-story mockups, and the review-overlay's **EPSS** index (Epic · Persona · Scenario · State) is rebuilt from *semantics*, not routes: a **Scenario** is the acceptance-scenario's "When" (a task), a **State** is its "Given" (the precondition), and the route is just where it starts — killing the meaningless one-entry-per-URL palette. The universal UI-Stack states (loading/empty/error) collapse into one **Foundations** home instead of repeating per page; pages render **data-driven** so the State seeds the data, not a layout flag. Specs carry an `epic` theme (menu-emitted, grouping scenarios across personas). The model is documented for agents in [`docs/EPSS.md`](./docs/EPSS.md). Dogfooded building the SC Dashboard (`dash`) mock on a hosted box.
...
|---|---|---|
| `@slowcook-ai/cli` | `0.22.0` (latest) | whole-app LCR `vibe` (plan→schema→seed→app) · EPSS manifest from acceptance_scenarios + Foundations home · `epic` plumbing · on top of GUCDI `menu`/`trace`/`eye`/`gate` |
| `@slowcook-ai/review-overlay` | `0.9.7` (latest) | framework-agnostic (no Next.js coupling) · EPSS jump palette · multi-person LCR review: device-flow identity, `[LCR]` issues, Docs studio, persona pane, apply-gate |
| `@slowcook-ai/core` | `0.16.0` (latest) | spine schema on `Spec` (`epic` · `prd_ref` / `data_contract` / `open_questions` / `fidelity`) + `ForgeReviewerAuth` seam |
| `@slowcook-ai/forge-github` | `0.14.0` (latest) | `GitHubReviewerAuth` (device flow) · `slowcook-trace` workflow · `slowcook-eye` / `slowcook-gate` |
| `@slowcook-ai/llm-anthropic` | `0.19.0` (latest) | `MENU_SYSTEM` / `MenuStoryDraft` emits `epic` (EPSS theme) · PRD→stories prompt + plate-mode contract |
...
**Adopting:** `npm install @slowcook-ai/cli@0.22.0`. Pin in your `.brewing/slowcook-cli-version`. First-run consumers get the full knowledge-layer bedrock automatically via `slowcook init`.
