...
6. **Would the PM plausibly have a non-default opinion?** A question with multiple defensible technical options is NOT enough reason to ask — the bar is "would the PM, reading this, have a preference?" Schema-design choices the PM would shrug at ("nullable timestamp column vs derived completeness check vs enum status") = DECIDE. Pick the cleanest option, put the choice + one-line rationale in \`<details>\`. Bad (observed): asking "How is incomplete registration detected? (a) \`registration_completed_at TIMESTAMPTZ NULL\` column vs (b) derived vs (c) enum" when the agent has already pre-picked (a) in the rationale. Good: silently pick (a), document the choice, move on.
...
- **A cited contract ANSWERS the question — do not re-open it.** When a predecessor spec you cite already fixes the value a question is about (a threshold, a token, a text format, an endpoint field), you have the answer: DECIDE by that contract and record the decision in \`<details>\`, or — if deviating is genuinely defensible — ask a DEVIATION question whose **(recommended)** option IS the shipped value, naming the spec. Never offer an option set that excludes the contract you just cited. Live failure: the agent cited story-018's bands (≥6 / 3–5 / ≤2) in its lineage note, then asked the PM to pick a "low" threshold from options none of which was a band boundary — inviting a second, conflicting definition of one quantity.
- **Values you attribute to a predecessor must be COPIED, not recalled.** If the spec says a value comes from story-N, it must be byte-identical to story-N's. Same failure, one step later: the emitted spec wrote \`var(--tint-inspire)\` where the cited spec ships \`var(--tint-amuse)\`.
