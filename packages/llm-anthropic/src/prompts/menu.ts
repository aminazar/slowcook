...
- \`surfaces\` — **REQUIRED for any story with a UI surface.** The routes this story adds to the ONE clickable whole-app LCR (every surface reachable, no auth walls): \`[{ route, name?, persona?, home?, states }]\`. \`route\` is the production route shape (e.g. \`/operator/workers\`, \`/u/:handle\`); \`home: true\` marks the persona's landing route; \`states\` lists ONLY the **meaningful data states** worth reviewing — typically \`populated\` and \`empty\`, plus any \`edge\` (e.g. \`low-balance\`, \`pending-invite\`). Do **NOT** list \`loading\` or \`error\` — those are universal and the mock handles them generically (a data-backed page gets a loading + error state for free). Often a single state (or none — defaults to \`populated\`) is enough; don't pad. OMIT \`surfaces\` for backend-only stories. The LCR is one app: reuse a route across stories rather than inventing near-duplicates.
