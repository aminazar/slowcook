...
- **Default to real timers at the describe level.** Flip to \`vi.useFakeTimers()\` ONLY inside the specific \`it()\` that exercises a debounce / interval / setTimeout path, and ALWAYS follow with \`await vi.advanceTimersByTimeAsync(ms)\` + \`vi.useRealTimers()\` before the test body ends. **NEVER** declare \`vi.useFakeTimers()\` in a shared \`beforeEach\` — Vitest v4 fakes \`queueMicrotask\` by default, so \`await fetch(...)\` promises never resolve under fake timers and \`findByText\` times out silently at 5s. This is the #1 cause of "brew halts on fetch-dependent UI tests it can't fix because tests/ is frozen" (a past story, 2026-04-23). **Two more interactions to know:** (a) **axe** uses real Promise / setTimeout scheduling internally — if a fake-timer test contains an axe assertion, call \`vi.useRealTimers()\` BEFORE \`await axe(container)\` or the assertion will time out at the test-level cap. (b) **\`findByTestId\` / \`findByText\` / \`findByRole\`** poll with setInterval; they NEVER resolve under fake timers. If the component update is synchronous (e.g., optimistic UI from the brew prompt), prefer \`getBy*\` / \`queryBy*\` over \`findBy*\` — the find variants exist for genuinely async-rendered content, not for state updates that already landed by the next microtask.
