...
1. **Extend existing files; don't create parallels.** If the PM's feedback asks to amend the pin button and a \`ItemListPage.tsx\` already exists in the index, edit that file. Don't create a new \`ItemListPageV2.tsx\` or \`PinnedItemList.tsx\`. The plate divergence problem is the same as vibe's — the past dogfood stall traced to vibe creating \`ItemListPageV2\` while tests imported \`ItemListPage\`. Plate amending into a third name would compound the problem.
...
PM asked the strip's pinned cards to render with a sticker-style border. ItemCard
currently has only one border style. Recommend a new optional prop:
  - \`variant?: "default" | "pinned"\`
  - "pinned" applies \`border-2 border-coral border-dashed\`
This plate run does NOT modify ItemCard; PM should approve before brew applies.
</component_change_request>

<plate_summary>
- Changed strip card padding from p-3 to p-4 (per a PM comment: "cards feel cramped")
- Updated story-N scenario fixture #2 to be a podcast (per the screenshot pointing at the second card with "this should be audio not text")
- Surfaced a component-change request for ItemCard pinned-variant border (saw the screenshot annotation but plate can't modify shared primitives)
</plate_summary>
\`\`\`
...
2. **Reuse the existing component names from the mockup branch.** If the user message lists \`mock/src/components/members/PinnedStrip.tsx\`, your amendment edits THAT file — don't invent a new file like \`PinnedItemsStrip.tsx\` to hold the same logic. Inventing parallel components creates orphan files brew has to reconcile.
3. **NEVER use \`.js\` extensions in TypeScript imports.** Next/Turbopack uses bundler module resolution, which doesn't auto-resolve \`./foo.js\` → \`./foo.ts\`. Write extensionless: \`import story017 from "../../scenarios/story-N"\`.
