...
// Mirrors stack-ts's discover.ts — same DiscoveryResult shape so CLI's
// manifest record/verify works unchanged regardless of stack.
...
export interface DiscoverOptions {
  ...
  cwd: string;
  /** Max bytes to buffer per command (defaults to 64MB). */
  maxBuffer?: number;
...
/**
 * Run every declared test suite's discover_command (canonically
 * `forge test --list --json`) and return a unified list of tests plus
 * per-suite metadata. A failing suite does not fail the whole
 * discovery — the error is captured in the result so the caller can
 * decide how strict to be.
 */
export function discoverTests(
  config: SolidityStackConfig,
