...
export const COMMANDS: ReadonlyArray<CommandEntry> = [
  ...
  {
    name: "vibe",
    usage: "slowcook vibe (plan | schema [--stdout] | seed [--dry-run] | app [--force] | journeys [--concept <path>] [--dry-run] | tell [--journey <id>] [--seed <n>] [--dry-run] | check [--regen-worlds] [--dry-run] | --spec <id> [--owner <login>] [--dry-run]) [--cwd <path>]",
  ...
  {
    name: "analyze",
    usage: "slowcook analyze --spec <id> [--cwd <path>] [--json]",
    description: "Deterministic cross-spec + as-built consistency gate for one spec: same endpoint declared with contradictory request fields/responses in another active spec (param names normalized), two specs creating the same table, and cited entities or altered tables that exist nowhere. Exit 1 on findings; taste's spec reviews receive the findings as evidence. Adapted from Spec Kit (github/spec-kit, MIT).",
    group: "pipeline",
    status: "alpha",
  },
  {
