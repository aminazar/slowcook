...
export async function initEntities(argv: string[], _cliVersion: string): Promise<void> {
  ...
  // Mock-side bridge: when mock/ exists, drop a re-export shim at
  // mock/src/lib/entities.ts so `@/lib/entities` resolves under mock's
  // tsconfig (where `@/*` → `mock/src/*`). Without this, vibe + plate
  // can't import canonical entity types and silently fall back to inline
  // redeclaration. The relative path from mock/src/lib/ to src/lib/entities/
  // is `../../../src/lib/entities/index.js` (2 ups from mock/src/lib to
  // mock/, 1 more to repo root, then src/lib/entities).
  const mockDir = join(args.cwd, "mock");
  if (existsSync(mockDir)) {
    const mockShimDir = join(mockDir, "src/lib");
    const mockShimPath = join(mockShimDir, "entities.ts");
    const relPath = relative(args.outDir, ".") || ".";
    // outDir is "src/lib/entities" — relative from "mock/src/lib" to
    // "src/lib/entities/index.js" walks up 3 levels then descends.
    const importPath = `../../../${args.outDir}/index.js`;
    const shimContent = renderMockShim(importPath);
    const shimAction = decideAction(mockShimPath, shimContent);
    emitted.push({ path: mockShimPath, content: shimContent, action: shimAction });
    if (!args.dryRun && (shimAction === "create" || shimAction === "update")) {
      mkdirSync(mockShimDir, { recursive: true });
      writeFileSync(mockShimPath, shimContent, "utf8");
    }
    // Silence unused: relPath placeholder for future use if outDir is
    // configured non-standard. Today we always use args.outDir directly.
    void relPath;
  }
...
function renderMockShim(importPath: string): string {
  return `/**
 * AUTO-GENERATED by \`slowcook init entities\`. Do not hand-edit.
 *
 * Re-exports the consumer's prod entity types under the mock app's
 * \`@/\` path alias. Without this shim, mock components written by vibe
 * + plate can't import \`@/lib/entities\` (which only resolves under the
 * top-level tsconfig where \`@/*\` → \`./src/*\`); they'd silently fall
 * back to redeclaring entity shape inline. The shim makes the entity
 * layer reachable from both sides.
 */
