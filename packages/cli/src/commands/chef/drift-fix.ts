 ...
 */
function publishChefBrewHaltFix(repoRoot: string, storyId: string, rationale: string): void {
  ...
  try {
    existingPr = execSync(
      `gh pr list --repo "${repoSlug}" --head "${branch}" --json number --jq '.[0].number // ""'`,
      { encoding: "utf8" },
    ).trim();
  ...
  try {
    execSync(
      `gh pr create --repo "${repoSlug}" --base main --head "${branch}" --title "${title.replace(/"/g, '\\"')}" --body-file "${bodyFile}"`,
      { stdio: "inherit" },
    );
    console.log(`  brew-halt publish: opened PR for ${branch}`);
  } catch (e) {
    console.warn(`  warn: brew-halt PR create failed: ${(e as Error).message.slice(0, 200)}`);
  }
}
