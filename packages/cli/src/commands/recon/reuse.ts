...
/**
 * 0.19.0-α.10 (#85) — detect "this file is auto-generated, exclude
 * from refactor-candidate scanning." Pure: takes file content, returns
 * true if any of these markers appears in the first 400 characters:
 *
 *   `@slowcook-template`     — explicit "slowcook generated me" marker
 *   `@slowcook-stub`         — placeholder waiting for brew to overwrite
 *   `@generated`             — common convention (Facebook/Meta tooling)
 *   `AUTO-GENERATED`         — case-insensitive plain English
 *   `auto-generated by`      — common phrasing in tool output
 *   `Do not edit by hand`    — common phrasing in templated files
 *
 * 400 chars is enough to capture a typical file header without scanning
 * the whole body. Markers later in the file are deliberately ignored
 * (e.g., a comment in the middle of a file mentioning "auto-generated"
 * shouldn't kill the file's signal).
 */
export function isAutoTemplateFile(content: string): boolean {
  const head = content.slice(0, 400);
  if (head.includes("@slowcook-template")) return true;
  if (head.includes("@slowcook-stub")) return true;
  if (head.includes("@generated")) return true;
  if (/auto-generated/i.test(head)) return true;
  if (/do not edit by hand/i.test(head)) return true;
  return false;
}

/**
 * 0.19.0-α.10 (#85) — match a single file path against a glob-ish
 * exclusion pattern. Same shape as the refactor command's matchesScope
 * (kept inline rather than cross-package-imported to avoid coupling
 * recon to the refactor module).
 *
 * Supported:
 *   - exact match: "src/foo.ts"
 *   - directory prefix: "src/lib/entities/"  (matches anything under)
 *   - star at end: "src/lib/data/*"  (one segment after)
 *   - double-star: "src/lib/entities/**"  (any depth)
 *   - filename glob: "*.mock.ts"  (matches any path ending with)
 */
export function matchesExcludePattern(filePath: string, pattern: string): boolean {
  ...
  if (pattern.startsWith("*.")) {
    const ext = pattern.slice(1); // ".mock.ts"
