...
describe("isAutoTemplateFile (#85)", () => {
  it("detects @slowcook-template marker", () => {
    expect(isAutoTemplateFile("// @slowcook-template — slowcook-generated\nexport const x = 1;")).toBe(true);
  });

  it("detects @slowcook-stub marker", () => {
    expect(isAutoTemplateFile("// @slowcook-stub story-016\nthrow new Error('not impl');")).toBe(true);
  });

  it("detects @generated marker", () => {
    expect(isAutoTemplateFile("/* @generated by codegen */\nexport const x = 1;")).toBe(true);
  });

  it("detects 'AUTO-GENERATED' (case-insensitive)", () => {
    expect(isAutoTemplateFile("// AUTO-GENERATED. Do not edit.\nexport const x = 1;")).toBe(true);
    expect(isAutoTemplateFile("// auto-generated by foo\nexport const x = 1;")).toBe(true);
  });

  it("detects 'do not edit by hand' phrasing", () => {
    expect(isAutoTemplateFile("// Do not edit by hand — regenerate via slowcook init\nexport const x = 1;")).toBe(true);
  });

  it("does NOT match for normal hand-authored code", () => {
    expect(isAutoTemplateFile(`import { useState } from "react";\nexport function Foo() { return null; }`)).toBe(false);
  });

  it("ignores marker after first 400 chars (header-only check)", () => {
    const padding = "x".repeat(450);
    expect(isAutoTemplateFile(`${padding}\n// @slowcook-template`)).toBe(false);
  });
});

describe("matchesExcludePattern (#85)", () => {
  it("exact match", () => {
    expect(matchesExcludePattern("src/foo.ts", "src/foo.ts")).toBe(true);
  });

  it("directory prefix with trailing slash", () => {
    expect(matchesExcludePattern("src/lib/entities/foo.ts", "src/lib/entities/")).toBe(true);
    expect(matchesExcludePattern("src/lib/data/foo.ts", "src/lib/entities/")).toBe(false);
  });

  it("single-star one segment", () => {
    expect(matchesExcludePattern("src/lib/data/foo.ts", "src/lib/data/*")).toBe(true);
    expect(matchesExcludePattern("src/lib/data/sub/foo.ts", "src/lib/data/*")).toBe(false);
  });

  it("double-star any depth", () => {
    expect(matchesExcludePattern("src/lib/entities/foo/bar.ts", "src/lib/entities/**")).toBe(true);
    expect(matchesExcludePattern("src/lib/entities/foo.ts", "src/lib/entities/**")).toBe(true);
  });

  it("filename glob (*.mock.ts)", () => {
    expect(matchesExcludePattern("src/lib/data/pins.mock.ts", "*.mock.ts")).toBe(true);
    expect(matchesExcludePattern("anywhere/depth/foo.mock.ts", "*.mock.ts")).toBe(true);
    expect(matchesExcludePattern("src/lib/data/pins.ts", "*.mock.ts")).toBe(false);
  });

  it("returns false for unsupported pattern shapes", () => {
    // Star in the middle isn't supported — returns false (won't accidentally match)
    expect(matchesExcludePattern("src/foo/bar.ts", "src/*/bar.ts")).toBe(false);
  });
});
