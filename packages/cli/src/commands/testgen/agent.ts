...
import { discoverPackageDirs, buildHistoryIndex } from "../refine/history-index.js";
...
export function buildProjectContext(repoRoot: string): string {
  ...
  if (contextMd) {
    ...
    // sc#240 convention-mining: no hand-authored conventions doc — mine the
    // repo's own newest story tests as style exemplars. Five dogfood runs
    // produced five architectures before this; the repo's existing tests are
    // the ground truth for DI style, import extensions, and mock wiring.
    const exemplars = mineTestExemplars(repoRoot);
    if (exemplars) {
      bits.push(
        "### Existing test conventions (no .brewing/context.md — MATCH the style below)\n" +
        "These are the repo's own newest tests. Match their DI style, import extension style, " +
        "export style, mock wiring, and directory layout EXACTLY — do not introduce a different architecture.\n"
      );
      bits.push(exemplars);
    }
  }
...
/**
 * sc#240 — up to two newest story-test files from the history index,
 * excerpted (first 60 lines each) as style exemplars for the prompt.
 */
export function mineTestExemplars(repoRoot: string): string | null {
  const idx = buildHistoryIndex({ repoRoot });
  const storyTests = idx.test_files
    .filter((f) => /story-\d+/.test(f.file))
    .sort((a, b) => b.file.localeCompare(a.file))
    .slice(0, 2);
  if (storyTests.length === 0) return null;
  const parts: string[] = [];
  for (const f of storyTests) {
    try {
      const text = readFileSync(join(repoRoot, f.file), "utf8");
      const excerpt = text.split("\n").slice(0, 60).join("\n");
      parts.push(`#### \`${f.file}\` (excerpt)\n\n\`\`\`ts\n${excerpt}\n\`\`\``);
