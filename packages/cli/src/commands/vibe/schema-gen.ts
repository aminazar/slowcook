...
// A real in-browser SQLite (sql.js); the query layer (queries.ts) runs
// Drizzle/SQL, so mock→prod is a data-source swap, not a rewrite.
//
// WORLDS (storyteller stage): the boot state is a named world.
//   "empty"   → schema only (the storyteller's law: stories start at nothing)
//   "default" → the LCR seed (worlds/default.ts, wraps seed.ts)
//   any other → a module in ./worlds/ — typically a snapshot a walk produced
// Select via ?world=<id> (persisted) or localStorage "slowcook_world".
//
// WALKER SEAM: after boot, window.__slowcook = { data, snapshot } — the
// storyteller (vibe tell) and the mock-checker (vibe check) assert
// acceptance-state deltas through it. Requires deps: drizzle-orm, sql.js.
// Do not hand-edit.
...
const worldModules = import.meta.glob("./worlds/*.ts");

export function currentWorld(): string {
  try {
    const q = new URL(window.location.href).searchParams.get("world");
    if (q) { localStorage.setItem("slowcook_world", q); return q; }
    return localStorage.getItem("slowcook_world") ?? "default";
  } catch { return "default"; }
}

let _sqlite: { export(): Uint8Array } | null = null;
let _dbPromise: Promise<DB> | null = null;

async function seedWorld(db: DB, world: string): Promise<void> {
  if (world === "empty") return;
  const loader = worldModules[\`./worlds/\${world}.ts\`];
  if (!loader) {
...
/** Deterministic FNV-1a hash of the full database image — the walkers'
 *  changed-at-all floor; acceptance asserts use \`data\` queries. */
function snapshotHash(): string {
  if (!_sqlite) return "unbooted";
  const bytes = _sqlite.export();
  let h = 0x811c9dc5;
  for (let i = 0; i < bytes.length; i++) { h ^= bytes[i]!; h = Math.imul(h, 0x01000193); }
  return (h >>> 0).toString(16);
}

/** Lazily boot the in-memory SQLite, materialise the schema, seed the
 *  selected world once, and install the walker seam. */
export function getDb(): Promise<DB> {
  if (!_dbPromise) {
    _dbPromise = (async () => {
      ...
      await seedWorld(db, currentWorld());
      if (typeof window !== "undefined") {
        const data = await import("./queries");
        (window as unknown as { __slowcook: unknown }).__slowcook = {
          data,
          world: currentWorld(),
