...
`@slowcook-ai/review-overlay` 0.9.0 (overlay-only; additive). Reviewing a mock means reaching every UI state of every surface fast. A consumer can now point the overlay at a `testing-surfaces.json` manifest (new `testingSurfacesUrl` prop, or `NEXT_PUBLIC_SLOWCOOK_SURFACES_URL`) and the pill grows an **EPSS router** — **Epic ▸ Context ▸ Scenario ▸ State ▸ CTA** — so any action is ≤4 clicks. It lives **inside the pill (one artifact)**, shown only in **review mode** (a 2×2 dropdown router; nav mode stays a nimble pill). The mode toggle is a compact **nav/rev** switch, and the pill is width-capped so it stays tidy on desktop too (mobile-portrait friendly).
